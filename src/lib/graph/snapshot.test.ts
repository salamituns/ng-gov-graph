...
describe('resolveNigeriaGraph', () => {
	...
	it('fills sourced heads in a stored snapshot and replaces only a documented predecessor', async () => {
		...
		assert.equal(graph.nodes['ng-nalv-head'].people[0]?.name, 'Muhammad Mai Abubakar')
		assert.equal(graph.nodes['ng-nalv-head'].name, 'Director and Chief Executive Officer')
		assert.equal(graph.nodes['ng-bcda-head'].people[0]?.name, 'Abdulrazak Sa’ad Namdas')
		assert.equal(graph.nodes['ng-bcda-head'].people[0]?.handoverUnconfirmed, true)
	})
})
