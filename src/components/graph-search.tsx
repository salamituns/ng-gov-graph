...
export function GraphSearch({ gov, graph }: { gov: string; graph: CompiledGraph }) {
	...
	return <>
		...
		<dialog ref={dialog} className="graph-search" aria-label="Search government graph" onClose={() => setQuery('')} onClick={event => { if (event.target === event.currentTarget) event.currentTarget.close() }}>
			...
			{query.trim() && <div className="graph-search-results"><h2>Entities</h2>{hits.length ? <ul>{hits.map(node => <li key={node.id}><Link href={nodePath(gov, node)}><span className="graph-search-glyph" aria-hidden="true" /><span><strong>{node.name}</strong><small>{node.type.replaceAll('_', ' ')}{node.people[0] ? ` · ${node.people[0].name}` : ''}</small></span></Link></li>)}</ul> : <p>No matches. Try a person, office, or agency alias.</p>}</div>}
		</dialog>
	</>
}
